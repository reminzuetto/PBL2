...
void Manager::AddFilm()
{
    ...
    outFile.seekp(0);
    outFile << AmountOfFilm;
    inFile.close();
    outFile.close();
    cout << "Them phim thanh cong. Tong so phim moi la: " << AmountOfFilm << endl;
}
void Manager::DeleteFilm()
{
    ...
    cin >> x;
    if (x < 1 || x > AmountOfFilm)
    {
        cout << "Khong tim thay phim can xoa!" << endl;
        return 1;
    }
    Film temp;
    ...
    ofstream inFile;
    inFile.open("FilmList.txt",ios::out);
    inFile << this->AmountOfFilm;
...
void Manager::EditFilm()
{
    ...
    int countFilm;
    inFile >> countFilm;
    films.setSize(countFilm);
    for (int i = 0; i < countFilm; i++)
    {
        inFile >> temp;
        films[i].setData(temp);
    }
    inFile.close();
